"ประเทศไทยมีข้อมูลกฎหมายและประกาศหลายล้านฉบับ แต่ทำไมเราถึงยังไม่มี AI กฎหมายเก่งๆ หรือระบบค้นหาที่เข้าใจบริบทได้สักที?" คำตอบสั้นๆ คือ "ข้อมูล" ครับ.. ข้อมูลส่วนใหญ่ของเรายังติดอยู่ในรูปแบบ PDF หรือรูปภาพสแกน (Unstructured Data) ที่มนุษย์อ่านรู้เรื่อง แต่คอมพิวเตอร์และ LLM กลับมองเห็นเป็นเพียงความว่างเปล่า นี่จึงเป็นจุดเริ่มต้นของโครงการที่เกิดจากความร่วมมือระหว่างภาคประชาชนและฝ่ายนิติบัญญัติ โดยมีเป้าหมายเดียวคือการปรับรูปแบบเอกสารประวัติศาสตร์ของชาติ ให้กลายเป็นโครงสร้างพื้นฐานทางข้อมูล (Data Infrastructure) ที่พร้อมใช้งานจริง
...
## สถาปัตยกรรมระบบแบบ Zero Budget

เมื่อได้รับข้อมูลดิบมาแล้ว โจทย์ใหญ่ทางวิศวกรรมคือ "จะจัดเก็บและประมวลผลอย่างไรให้ยั่งยืน?" โดยไม่มีงบประมาณสำหรับ Cloud Server หรือ Data Center ราคาแพง ทีมงานจึงต้องออกแบบสถาปัตยกรรมที่เน้นความคุ้มค่าสูงสุด (Cost-Efficiency) โดยผสมผสานเครื่องมือที่เหมาะสมเข้าด้วยกัน

#### ทำไมต้อง Hugging Face

สำหรับการจัดเก็บและแจกจ่ายข้อมูล (Storage & Distribution) เราตัดสินใจเลือกใช้ **Hugging Face** เป็นฐานข้อมูลหลักแทนการใช้ Cloud Storage ทั่วไป เนื่องจากตอบโจทย์โครงการลักษณะนี้ที่สุด:
...
#### ขุมพลังหลังบ้าน: เปลี่ยน NAS ให้เป็น Server
...
* **Automation:** ตั้ง Schedule ให้ Container ทำงานอัตโนมัติในช่วงเวลาที่เหมาะสม (เช่น ตอนกลางคืน) เพื่อประมวลผลข้อมูลวันละนิดละหน่อยอย่างสม่ำเสมอ
* **Energy Efficient:** กินไฟน้อยกว่าการเปิด PC หรือ Server ทิ้งไว้มาก ช่วยลดต้นทุนระยะยาวให้เป็นศูนย์

![สถาปัตยกรรมแบบ "Zero Budget](https://www.openlawdatathailand.org/press/2025-12-31_blognone/3.jpg)


## การจัดการไฟล์ 1.3 ล้านฉบับบน Git: จากระบบล่มสู่สถาปัตยกรรม Hot/Cold Storage
...
1. **Git Index Bloat:** Git ไม่ได้ถูกออกแบบมาให้จัดการกับจำนวนไฟล์ (File Count) หลักล้านชิ้นใน Repository เดียว ทำให้ Index ของ Git มีขนาดใหญ่จนกินทรัพยากรเครื่องมหาศาล ส่งผลให้ Pipeline หยุดทำงาน (Freeze)
...
#### การจัดโครงสร้าง Directory (Hierarchical Structure)
...
## เปลี่ยนภาพให้เป็นข้อมูลด้วย OCR ภาษาไทย

#### ปัญหา Scanned PDF: เมื่อข้อมูลเป็นเพียง "รูปภาพ"

แม้จะบริหารจัดการไฟล์จำนวน 1.3 ล้านฉบับได้สำเร็จ แต่โจทย์ใหญ่ถัดมาคือ "รูปแบบของข้อมูล" เอกสารราชกิจจานุเบกษาจำนวนมาก (โดยเฉพาะช่วงก่อนปี พ.ศ. 2550) ถูกจัดเก็บในรูปแบบ **Scanned PDF** ซึ่งในทางคอมพิวเตอร์มองเห็นเป็นเพียงไฟล์รูปภาพ (Image) ไม่ใช่ข้อความ (Text) ทำให้นักพัฒนาไม่สามารถนำไปทำ Indexing เพื่อค้นหาคำสำคัญ (Keyword Search) หรือนำไปประมวลผลต่อด้วย NLP ได้ หากไม่ผ่านกระบวนการแปลงภาพเป็นข้อความเสียก่อน
...
2. **Legacy Fonts:** เอกสารยุคเก่าใช้พิมพ์ดีดหรือฟอนต์เฉพาะทาง (เช่น ตระกูล JS/DS ยุคแรก) ซึ่ง OCR Engine ทั่วไปที่เป็น Global Standard มักมีปัญหากับการตัดคำและการอ่านตัวอักษรไทยรูปแบบเก่าเหล่านี้
...
ทีมพัฒนาได้ออกแบบ Data Pipeline บน Synology NAS ให้ทำงานแบบ **Asynchronous Background Job** โดยค่อยๆ ส่งไฟล์ PDF ย้อนหลังเข้าสู่ API ของ iApp เพื่อประมวลผลทีละหน้าอย่างต่อเนื่อง (Continuous Processing) เพื่อไม่ให้กระทบกับ Traffic หลัก ผลลัพธ์ที่ได้จะถูกบันทึกกลับมาเป็นไฟล์ Text/JSON ที่นักวิจัยสามารถนำไปใช้งานต่อได้ทันที เปลี่ยนเอกสารประวัติศาสตร์ที่เคยเป็นเพียงรูปภาพ ให้กลายเป็น **Searchable Data** ที่ค้นหา
...
#### โจทย์ของการเข้าถึงข้อมูล (Data Retrieval Challenge)

เมื่อรวบรวมข้อมูลได้ระดับ 1.3 ล้านไฟล์ โจทย์สำคัญคือการออกแบบวิธีการเข้าถึง (Access Pattern) ให้นักพัฒนาสามารถเลือกดึงข้อมูลเฉพาะส่วนที่ต้องการได้โดยง่าย หากกองข้อมูลทั้งหมดรวมกัน นักพัฒนาที่ต้องการเพียงข้อมูล Text ปีปัจจุบันอาจต้องเสียเวลาดาวน์โหลดไฟล์ ZIP ขนาดใหญ่ทั้งถังเพื่อมาคัดแยกเอง ซึ่งไม่ตอบโจทย์การใช้งานจริง
...
#### การจัดโครงสร้าง Directory แบบแยกส่วน (Separation of Concerns)

เราจัดระเบียบข้อมูลบน Repository โดยแยกตามประเภทการนำไปใช้งาน (Use Cases) ดังนี้:
...
1. **Status File Trigger:** เมื่อ Data Pipeline บน NAS ประมวลผลเสร็จสิ้น (Fetch -> OCR -> Upload) ระบบจะสร้างไฟล์สรุปสถานะ (Status File) และ Commit ทับไฟล์เดิมบน Repository
...
2. **Transparency:** การเปิดเผย Source Code เป็น Open Source ช่วยให้ภาคประชาชนสามารถตรวจสอบ (Audit) หรือร่วมพัฒนา (Contribute) ได้
3. **Sustainability:** การใช้ GitHub Pages ไม่มีค่าเช่า Server ให้เป็นศูนย์ (Zero Cost) ทำให้โครงการดำเนินต่อไปได้อย่างยั่งยืน

#### ปรากฏการณ์วันเปิดตัว: เมื่อ Share มากกว่า Like

เมื่อระบบพร้อมสมบูรณ์ ทีมงานได้ประกาศเปิดตัว (Beta Launch) ผ่านช่องทาง [Facebook Post](https://www.facebook.com/kriangkrai.chaonithi/posts/pfbid0VPNu3eF5EnyVwkS6qLVkmikXtecMcg55X4qZVzr1euaMZ6w3HQjFbWw8hH89osbEl) เพื่อแจ้งข่าวสารแก่ Community นักพัฒนา ผลปรากฏว่าได้รับความสนใจอย่างรวดเร็ว โดยสถิติที่น่าสนใจภายใน 24 ชั่วโมงแรกคือ **ยอดการแชร์ (Share) สูงกว่ายอดกดถูกใจ (Like) ถึงกว่า 2 เท่า** (900 Shares vs 400 Likes)

ในมุมมองการวิเคราะห์ข้อมูล ตัวเลขนี้บ่งชี้ว่าสังคมไม่ได้มองเนื้อหานี้เป็นเพียง Content ทั่วไป แต่มองเป็น **"Resource"** ที่มีคุณค่าและต้องเก็บรักษาไว้ใช้งาน สะท้อนให้เห็นถึงความต้องการ (Demand) ของนักพัฒนาไทยที่รอคอยโครงสร้างพื้นฐานทางข้อมูลที่มีคุณภาพมาอย่างยาวนาน
